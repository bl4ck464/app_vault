...
/*
|--------------------------------------------------------------------------
| APK VAULT — STORE ENGINE  v2 (Simplified)
|--------------------------------------------------------------------------
| Removed: category, sort, filters. Only search remains.
|--------------------------------------------------------------------------
*/
...
async function loadStore() {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(
                `app.json returned HTTP ${response.status}`
            );
        }
        ...
        if (
            ...
            console.warn(
                "APK Vault → Unexpected content type:",
                contentType
            );
        }
        ...
        console.log(
            `APK Vault → ${state.apps.length} apps loaded`
        );
        ...
        console.error(
            "APK Vault → Store loading failed:",
            error
        );
...
function createCard(app) {
    ...
    return `
        ...
        >
            <a
                class="app-card-main"
                href="${STORE.appPage}?id=${id}"
            >
                <div class="app-card-icon">
                    ${
                        app.icon
                            ? `<img
                                    src="${escapeAttr(app.icon)}"
                                    alt=""
                                    loading="lazy"
...
function renderFeatured() {
    ...
    const container =
        $("#featuredApps, .featured-apps, .featured-grid");

    if (!container) return;

    let featured =
        state.apps.filter(app => app.featured);

    if (!featured.length) {
        featured = state.apps.slice(0, 4);
    }
    ...
    const screenshots = (app.screenshots || [])
        .map((image, i) => `
            <img
                ...
                alt="${escapeAttr(app.name)} screenshot ${i + 1}"
                loading="lazy"
            >
        `)
        .join("");

    const redistributionText = app.redistributable
        ? "Allowed under distribution policy"
        : "Not permitted";
    ...
    container.innerHTML = `
        <div class="app-hero">
            <div class="app-main-icon">
                ${app.icon
                    ? `<img src="${escapeAttr(app.icon)}" alt="${escapeAttr(app.name)}">`
                    : ""
                }
            </div>
            <div class="app-main-info">
                ...
                <div class="app-stats">
                    <span>
                        <strong>${app.rating ? app.rating.toFixed(1) : "—"}</strong>
                        Rating
                    </span>
                    <span>
                ...
                <a
                    href="${escapeAttr(app.apk)}"
                    class="main-download"
                    download
                    ${!app.apk ? 'style="pointer-events:none;opacity:.5"' : ""}
                >
                    ${app.apk ? "Download APK" : "APK unavailable"}
                    ${app.apk ? icons.download : ""}
                </a>
            </div>
        </div>
        ...
        <section class="app-details">
            ...
            <aside class="app-info-panel">
                ...
                <div class="info-row">
                    <span>Developer</span>
                    <strong>${escapeHTML(app.developer)}</strong>
                </div>
                <div class="info-row">
                    <span>Version</span>
                ...
                <div class="info-row">
                    <span>Android</span>
                    <strong>${escapeHTML(app.android)}</strong>
                </div>
                <div class="info-row">
                    <span>Updated</span>
                ...
                <div class="info-row">
                    ...
                    <strong>${redistributionText}</strong>
                </div>
            </aside>
        </section>
        ...
        <section class="screenshots-section">
            ...
            <div class="screenshots">
                ${screenshots ||
                    `<p class="empty-state">Screenshots coming soon.</p>`
                }
            </div>
        </section>
        ...
        <section class="app-distribution">
            <div>
                ...
                <p>Redistribution may be allowed
                   under the APKVault distribution policy.</p>
            </div>
            <a href="distribution.html" class="hero-secondary">
                View Policy
            </a>
        </section>
    `;
...
function setupMenu() {
    ...
    const button =
        $("#menuBtn, #mobileMenu, .mobile-menu");
...
function renderAppsWithReveal(appList) {
